...
let expect t ?(fmatches=[]) actions action_default =
          ...
          List.find
            (fun (action_condition, _) ->
                match event, action_condition with
                  ...
                  | Line str, `Exact s ->
                      str = s
                  | Line str, `Suffix suffix ->
                      Stringext.ends_with str ~suffix
                  | Line str, `Prefix prefix ->
                      Stringext.starts_with str ~prefix
                  | Line str, `Contains substring ->
                      Stringext.contains str ~substring
                  | _ ->
                      false)
            actions
        in
  ...
  let expect_input_line cont =
    ...
    let lines =
      Stringext.split (t.prev ^ input_str) ~sep:'\n'
    in
